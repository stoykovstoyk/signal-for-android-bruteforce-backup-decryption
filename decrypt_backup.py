...
def read_backup_header(backup_file_: BinaryIO) -> HeaderData:
    """Read the header from the start of a Signal backup file."""
    # slaga tekushtata pozicia v binary data stream
    current_pos = 0

    # Read the first 4 bytes of the binary data stream at the current position and unpack them into an integer value
    backup_file_.seek(current_pos)
    length = struct.unpack(">I", backup_file_.read(4))[0]
    backup_frame = BackupFrame.FromString(backup_file_.read(length))
...
def derive_keys(passphrase: str, salt: bytes) -> Keys:
    ...
    sha512.update(salt)
    for _ in range(250000): #trqbva da e# for _ in range(250000)
        sha512.update(hash)
...
def main() -> None:
    ...
    parser.add_argument(
        "-password_list",
        help="File that contain possible passwords. One password at line",
    )
    ...
    if args.backup_file is None or args.password_list is None or args.output_directory is None:
        print("Please set all input parametere. Example on how to call the script:\n$ python3 decrypt_backup.py -backup_file signal.backup -password_list rockyou.txt -output_directory decrypted_directory")
        sys.exit(1)
        ...
        if not completed:
            ...
            try:
                for _ in decrypt_backup(
                    ...
                    args.backup_file, line.strip(), args.output_directory
                ):
